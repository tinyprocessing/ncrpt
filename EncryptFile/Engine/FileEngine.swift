...
class FileEngine: ObservableObject, Identifiable {
    ...
    func exportNCRPT(_ data: Data, filename: String, license: License = License()) {
        ...
        let fileURLLicense = libraryDirectory[0].appendingPathComponent(
            "/\(fileDirectory)/license.json"
        )
        ...
        do {
            ...
            try! byteHeader(fileURLNCRPTData, license.ext.lowercased()).write(
                to: fileURLNCRPT
            )

            do {
                try FileManager.default.removeItem(
                    atPath: (fileURLFolder.path()
