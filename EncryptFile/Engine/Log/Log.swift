...
public class Log: ObservableObject, Identifiable, TextOutputStream {
    ...
    public func write(_ string: String) {
        ...
        let log = fm.urls(for: .documentDirectory, in: .userDomainMask)[0].appendingPathComponent(
            "\(Bundle.main.bundleIdentifier!)/logs/ncrpt_\(dateFormatterGet.string(from: Date())).log"
        )
