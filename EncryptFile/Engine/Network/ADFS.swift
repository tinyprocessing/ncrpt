...
//        completionHandler(.cancelAuthenticationChallenge, nil)
    ...
    func jwt(completion: @escaping (_ success: Bool) -> Void) {
        if jwt != "" {
            ...
            let exp: Double = getDateFromJWT(jwt).timeIntervalSince1970
            if time < exp {
                completion(true)
                return
            }
        ...
        let urlString = "https://authory.ncrpt.io"
        let headers = [
        ...
        let dataTask = session.dataTask(with: request as URLRequest, completionHandler: { [self] data, response, error in
            if error != nil {
                completion(false)
            } else {
...
extension String {
    func base64Decoded() -> String? {
        ...
        if count % 4 <= 3 {
            st += String(repeating: "=", count: 4 - (count % 4))
        }
