...
open class Interface: CustomStringConvertible, CustomDebugStringConvertible {
    ...
    public static func createTestDummy(_ name: String, family: Family, address: String, multicastSupported: Bool,
                                       broadcastAddress: String?) -> Interface {
        return Interface(
            name: name,
            family: family,
            address: address,
    ...
    convenience init(data: ifaddrs) {
        ...
        self.init(name: String(cString: data.ifa_name),
                  ...
                  broadcastAddress: (broadcastValid && destinationAddress(data) != nil) ? Interface
                      .extractAddress(destinationAddress(data)) : nil)
    }
