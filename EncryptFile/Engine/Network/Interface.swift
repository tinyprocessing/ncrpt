...
import Foundation
        ...
        typealias Flags = Int
        func destinationAddress(_ data: ifaddrs) -> UnsafeMutablePointer<sockaddr>! {
            return data.ifa_addr
        }
        func socketLength4(_ addr: sockaddr) -> UInt32 {
            return UInt32(MemoryLayout<sockaddr>.size)
        }
    #else
        import Darwin
...
open class Interface: CustomStringConvertible, CustomDebugStringConvertible {
        ...
        if getifaddrs(&ifaddrsPtr) == 0 {
            ...
            while ifaddrPtr != nil {
                ...
                if addr?.sa_family == InetFamily(AF_INET)
                    || addr?.sa_family == InetFamily(AF_INET6)
                {
                    interfaces.append(
                        Interface(
                            data: (ifaddrPtr?
    ...
    convenience init(data: ifaddrs) {
        ...
        self.init(
            ...
            broadcastAddress: ((broadcastValid && destinationAddress(data) != nil)
                ? Interface.extractAddress(destinationAddress(data)) : nil)
        )
    }
    ...
    fileprivate static func extractAddress_ipv4(_ address: UnsafeMutablePointer<sockaddr>) -> String? {
        return address.withMemoryRebound(to: sockaddr.self, capacity: 1) { addr in
            ...
            if getnameinfo(
                &addr.pointee,
                socklen_t(socketLength4(addr.pointee)),
                &hostname,
    ...
    fileprivate static func inetNtoP(_ addr: UnsafeMutablePointer<sockaddr>, ip: UnsafeMutablePointer<Int8>)
        -> String?
    {
        return addr.withMemoryRebound(to: sockaddr_in6.self, capacity: 1) { addr6 in
            let conversion: UnsafePointer<CChar> = inet_ntop(
                AF_INET6,
                &addr6.pointee.sin6_addr,
                ip,
...
#if swift(>=5)
    extension Interface: Identifiable, Hashable {
