...
class RSA: ObservableObject, Identifiable  {
    ...
    func generatePairKeys() -> (String?, String?){
        do {
            ...
            let pem = PEMKeyFromDERKey(data: exportRSAPublicKeyToDER(rawPublicKeyBytes: testMethod as! NSData, keyType: kSecAttrKeyTypeRSA as String, keySize: 4096))
...
func exportRSAPublicKeyToDER(rawPublicKeyBytes: NSData, keyType: String, keySize: Int) -> NSData {
    ...
    let totalSizebitstringEncodingLength = encodeASN1LengthParameter(length: totalSize, buffer: &(headerBuffer[1]))
    ...
    let keyLengthBytesEncoded = encodeASN1LengthParameter(length: rawPublicKeyBytes.length+1, buffer: &(bitstringBuffer[1]))
