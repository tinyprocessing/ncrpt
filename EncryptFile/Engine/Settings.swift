...
class Settings: ObservableObject, Identifiable {
    ...
    var max_log = 15 // Mb
    var log_time_expire = 30 // Days
    var log_allow = true

    var email = "help@ncrpt.io"
    var server = "https://secure.ncrpt.io"
                        ...
                        LogsDirectoryContents.forEach { itemInLogs in
                            if (itemInLogs.localizedName ?? "").contains(".zip") {
                                log.debug(
                                    module: "Settings",
                                    type: #function,
                                    object: "clean - \(itemInLogs.localizedName ?? "*filename*")"
                                )
                                do {
    ...
    func alert(title: String, message: String, buttonName: String = "ok") {
        ...
        DispatchQueue.main.async {
            UIApplication.shared.windows.first?.rootViewController?.present(
                self.alertView(title: title, message: message, buttonName: buttonName),
                animated: true
            )
        }
    }
