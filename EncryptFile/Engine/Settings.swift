...
class Settings: ObservableObject, Identifiable {
    ...
    func shareLogs() {
        ...
        let fileLogsZIP =
            path + "/\(Bundle.main.bundleIdentifier!)"
            + "/ncrpt_\(currentDateString)_log.zip"
        ...
        do {
            ...
            do {
                try FileManager.default.removeItem(
                    atPath: (urlLogsZIP.path().removingPercentEncoding)!
                )
                try fileManager.zipItem(at: urlFolder, to: urlLogsZIP)
                share(items: [urlLogsZIP])
                log.debug(
        ...
        do {
            ...
            directoryContents.forEach { item in
                log.debug(
                    ...
                    type: #function,
                    object:
                        "clean - \(item.localizedName ?? "*filename*")"
                )
                                ...
                                log
                                    .debug(
                                        ...
                                        type:
                                            #function,
                                        object:
                                            "clean - \(itemInLogs.localizedName ?? "*filename*")"
                                    )
                                do {
                                    try FileManager
                                        .default
                ...
                if item.typeIdentifier == "public.folder" && !item.isNCRPT
                    && !name.contains("ncrpt.io.iosviewer")
                {
    ...
    func cleanLogs() {
        ...
        var cacheURL = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first!
            .absoluteString
        ...
        do {
            // Get the directory contents urls (including subfolders urls)
            let directoryContents = try FileManager.default.contentsOfDirectory(
                at: URL(fileURLWithPath: cacheURL),
                includingPropertiesForKeys: nil,
                options: []
            )
