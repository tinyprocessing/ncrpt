    ...
    var body: some View {
        VStack(spacing: 10) {
            ...
            if self.api.ui == .pin {
                Button(
                    action: {
                        NCRPTWatchSDK.shared.authorization
                            .passwordRecover =
                            true
                        withAnimation {
                            self.api.ui = .auth
    ...
    func isCorrectCode(pinCode: String) -> Bool {
        ...
        if bool {
            ...
            if tCodeArr[0] == tCodeArr[1] && tCodeArr[1] == tCodeArr[2]
                && tCodeArr[2] == tCodeArr[3]
                && tCodeArr[3] == tCodeArr[4]
            {
                return false
            }
            //константы
            switch tCodeArr {
            case [8, 9, 0, 1, 2], [2, 1, 0, 9, 8], [7, 5, 3, 1, 4], [9, 5, 1, 3, 6],
                [8, 6, 2, 4, 5], [6, 2, 4, 8, 5],
                [2, 4, 8, 6, 5], [5, 2, 4, 8, 6], [5, 8, 6, 2, 4],
                [3, 5, 7, 9, 6], [1, 5, 9, 7, 4], [7, 5, 3, 6, 9],
                [9, 5, 1, 4, 7], [9, 6, 3, 5, 7], [7, 4, 1, 5, 9]:
                return false
            default:
                break
            }
        ...
        if self.pin.count == 5 && self.api.ui == .pinCreate {
            ...
            case true:
                if self.pin == self.pinRepeat {
                    ...
                    self.api.authorization.savePasscode(
                        passcode: self.pin.joined()
                    )
                }
                else {
                    self.pin.removeAll()
                    ...
                    DispatchQueue.main.async {
                        Settings.shared.alert(
                            title: "Error",
                            message:
                                "The pin code does not match the one entered earlier",
                            buttonName: "Repeat"
                        )
                    }
                }
                ...
                if self.wrongAttemptCount == 2 {
                    ...
                    DispatchQueue.main.async {
                        Settings.shared.alert(
                            title: "Error",
                            message:
                                "You need to perform authorization",
                            buttonName: "Okey"
                        )
